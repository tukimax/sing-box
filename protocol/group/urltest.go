...
func (s *URLTest) pickOutbounds() ([]adapter.Outbound, error) {
	...
	for i, tag := range s.uses {
		provider, loaded := s.outbound.OutboundProvider(tag)
		if !loaded {
...
func (s *URLTest) Start() error {
	if s.useAllProviders {
		uses := []string{}
		for _, provider := range s.outbound.OutboundProviders() {
	...
	group, err := NewURLTestGroup(s.ctx, s.outbound, s.logger, outbounds, s.link, s.interval, s.tolerance, s.fallback, s.idleTimeout, s.interruptExternalConnections)
	if err != nil {
		return err
	}
	s.group = group
...
func NewURLTestGroup(ctx context.Context, outboundManager adapter.OutboundManager, logger log.Logger, outbounds []adapter.Outbound, link string, interval time.Duration, tolerance uint16, fallback URLTestFallback, idleTimeout time.Duration, interruptExternalConnections bool) (*URLTestGroup, error) {
	if interval == 0 {
...
func (g *URLTestGroup) urlTest(ctx context.Context, force bool) (map[string]uint16, error) {
	...
	for _, detour := range g.outbounds {
		...
		p, loaded := g.outboundManager.OutboundWithProvider(realTag)
		if !loaded {
			continue
		}
