...
func NewManager(logger log.ContextLogger, outbound adapter.OutboundManager) *Manager {
	return &Manager{
		logger:                logger,
		outbound:              outbound,
...
func (m *Manager) New(ctx context.Context, router adapter.Router, logger log.ContextLogger, options option.OutboundProvider) (adapter.OutboundProvider, error) {
	if options.Path == "" {
...
func (m *Manager) Create(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.OutboundProvider) error {
	provider, err := m.New(ctx, router, logger, options)
	if err != nil {
		return err
	}
	m.access.Lock()
